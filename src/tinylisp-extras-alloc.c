...
/* (alloc N) to allocate N bytes */
L f_alloc(L t, L *e) {
  I a = 0;
  L x = evarg(&t,e,&a);
  unsigned long long p = (unsigned long long)calloc((int)x, 1);
...
/* (poke mem offset value) to set the byte 'value' at mem + offset */
L f_poke(L t, L *e) {
  I a = 0;
  L address = evarg(&t,e, &a);
  int offset = (int)evarg(&t,e, &a);
  int value = (int)evarg(&t,e, &a);
...
/* (dump mem count) to t the byte 'value' at mem + offset */
L f_dump(L t, L *e) {
  I a = 0;
  L address = evarg(&t,e, &a);
  int count = (int)evarg(&t,e, &a);
