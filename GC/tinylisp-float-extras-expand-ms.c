/* tinylisp-float-extras-expand-ms.c more extras + expand hygienic macros + mark-sweep GC by Robert A. van Engelen 2026 */
...
#define L float
#define Z (8*sizeof(I)) /* number of bits of type I, for bits[] bit array operations */
...
I used(I i) { return bits[i/2/Z]&(1<<i/2%Z); }
void mark(I i) { bits[i/2/Z] |= 1<<i/2%Z; }
...
L atom(const char *s) {
 ...
 if (i == hp) {
  if ((hp += strlen(s)+1)+8 > lp<<2) ms(nil);
  memmove(A+i,s,hp-i);
 }
 return box(ATOM,i);
}
...
L cons(L x,L y) {
 ...
 if ((MS == 1 && !(fn&(fn+1))) || (MS == 2 && !(fn&(fn+1)) && fn != fl) || MS > 2 || hp+8 > fp<<2) ms(p); else lomem(i);
 return p;
}
...
L f_lt(L t,L *e) {
 I a = 0; L x,y;
 for (x = evarg(&t,e,&a); isarg(&t,e,&a,&y); x = y) if (!lt(x,y)) return nil;
 return tru;
}
 ...
 *p = t;                                        /* tail of s is t */
 ...
 if (hp+k+9 > lp<<2) err(4,nil);                /* ERR 4 if the heap space is not large enough */
...
L f_gt(L t,L *e) {
 I a = 0; L x,y;
 for (x = evarg(&t,e,&a); isarg(&t,e,&a,&y); x = y) if (!lt(y,x)) return nil;
 return tru;
}
...
int main(int argc,char **argv) {
 I i; printf("tinylisp-float-extras-expand-ms");
