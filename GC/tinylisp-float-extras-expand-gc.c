...
L atom(const char *s) {
 ...
 return i == hp && ((hp += strlen(s)+1)+8 > lp<<2 || !memmove(A+i,s,hp-i)) ? err(4,nil) : box(ATOM,i);
}
...
L cons(L x,L y) {
 ...
 if (TEST || hp+8 > fp<<2) ms(p); else lomem(i);
 return p;
}
